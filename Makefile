...
clean: ## Cleanup the repository
	$(GIT) clean -fX ./

# Go vendor, I think I should remove this one "someday"
vendor: ## Go vendor, I think I should remove this one "someday"
...
# Utility functions
need_root: ## Utility functions need root
	@[ "$(shell id -u)" -eq "0" ] || exit 1

not_root: ## Utility functions not root
	@[ "$(shell id -u)" != "0" ] || exit 1

# Database related
database-clean: ## Database related
	need_root
...
swagger-to-go: ## swagger-to-go
	$(INSTALL) ./cmd/swagger-to-go

generators: ## generators protoc gen wrapper and model
	$(INSTALL) ./cmd/protoc-gen-wrapper
	$(INSTALL) ./cmd/protoc-gen-model

proto: ## run proto
	$(BIN)/prototool $(BIN)/protoc-gen-go $(BIN)/protoc-gen-grpc-gateway $(BIN)/protoc-gen-swagger $(BIN)/protoc-gen-grpchan $(BIN)/protoc-gen-gogo generators
...
swagger: ## swagger to go proto
	swagger-to-go proto $(addsuffix -swagger,$(dir $(wildcard $(ROOT)/modules/*/)))

code-gen: ## swagger proto
	swagger proto

# Migrations
tools-migration: ## tools Migrations
	$(BIN)/go-bindata $(addsuffix -migration,$(dir $(wildcard $(ROOT)/modules/*/)))
...
test-full: ## test full
	test-database-setup test

build: ## Building all binaries
...
triger: ## triger didebaan-cli
	$(BIN)/didebaan-cli
	@$(BIN)/didebaan-cli

start: ## start docker compose up
	docker-compose up
