...
func TestNewRedisTokenProvider(t *testing.T) {
	...
	s, err := NewJWTTokenProvider("invalid", PublicKeyTest)
	require.Error(t, err)
	require.Nil(t, s)

	s, err = NewJWTTokenProvider(PrivateKeyTest, testInvalid)
	require.Error(t, err)
	require.Nil(t, s)

	s, err = NewJWTTokenProvider(PrivateKeyTest, PublicKeyTest)
